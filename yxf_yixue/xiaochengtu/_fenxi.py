...
class Chuantongfenxi:
    ...
    def fenxi(self, pan, yonggong='中'):
        self.pan = pan
        self.pan['标签'] = '传统分析'
        self.lunar = self.pan['农历']
        self.ganzhi = self.pan['干支']
        self.Bagua = self.pan['八卦']
        self.Liushisigua = self.pan['六十四卦']
        self.Pan = self.pan['盘']
        self.dongyao = self.pan['动爻']
        self.pan['传统分析'] = {'正推': {}, '触类': {}, '旁通': {}}
        self._zhengtuifa(yonggong)
        self._pangtongfa(yonggong)
        self._chuleifa(yonggong)
        return self.pan

    def _zhengtuifa(self, yonggong):
                ...
                if int(self.Pan[gong]['宫数']) == luogong_guagong:
                    if int(self.Pan[gong]['宫数']) not in tongji:
                        ...
                        if tongji[self.Pan[gong]['宫数']] == 1:
                            tongji[self.Pan[gong]['宫数']] += 1
                        break
        self.pan['传统分析']['正推']['宫数'] = luogong
        self.pan['传统分析']['正推']['宫卦'] = luogong_gua

    def _pangtongfa(self, yonggong):
        ...
        self.pan['传统分析']['旁通']['天通宫数'] = tianpan_pangtonggong
        self.pan['传统分析']['旁通']['天通宫卦'] = tianpan_pangtonggong_gua
        self.pan['传统分析']['旁通']['地通宫数'] = dipan_pangtonggong
        self.pan['传统分析']['旁通']['地通宫卦'] = dipan_pangtonggong_gua

    def _chuleifa(self, yonggong):
        ...
        self.pan['传统分析']['触类']['宫数'] = chuleigong
        self.pan['传统分析']['触类']['宫卦'] = chuleigong_gua
...
class Cecaifenxi(Chuantongfenxi):
    ...
    def fenxi(self, pan, yonggong='中'):
        ...
        self.pan = self.chuantongfenxi.fenxi(pan, yonggong)
        self.pan['标签'] = '测彩分析'
        self.lunar = self.pan['农历']
        self.ganzhi = self.pan['干支']
        self.Bagua = self.pan['八卦']
    ...
    def _canwuyishufa(self):
        ...
        num = []
        num.append(self._canwuyishufa_dangeshuzi(bengua_shang, bengua_xia, biangua_shang, biangua_xia))
        # 六个爻位纳数
        num1 = self._canwuyishufa_yaoweinashu(bengua_shang_code, bengua_xia_code)
        for i in num1:
            num.append(i)
        return num

    def _canwuyishufa_yaoweinashu(self, bengua_shang_code, bengua_xia_code):
        # 爻位纳数
        ...
        for i in range(0, 6):
            ...
            for j in self.Bagua.keys():
                ...
                if xia_code == eval(self.Bagua[j]['二进制']):
                    xia = j
            num[i] = self._canwuyishufa_dangeshuzi(shang, xia, shang, xia)
        return num

    def _canwuyishufa_dangeshuzi(self, bengua_shang, bengua_xia, biangua_shang, biangua_xia):
        # 本卦定大组
        num = ''
    ...
    def _dingweidanfa(self):
        ...
        zhengtuigua = self.pan['传统分析']['正推']['宫卦']
        ...
        wei = ''
        try:
            pangtonggua_tian = self.pan['传统分析']['旁通']['天通宫卦'][0]
            ...
            pass
        try:
            pangtonggua_di = self.pan['传统分析']['旁通']['地通宫卦'][0]
